...
When the "Connect to a server" radio button is selected, the following fields are available:
...
* <ins>MSSP</ins>: Checks the server for [MSSP](#mssp) information.
...
Note that not every text-based program is suitable for use in Galosh, as Galosh uses line-based input instead of sending individual keystrokes to the
...
When the "Run a program" radio button is selected, only one field is available:
...
Note that the password is stored in the [configuration file](config.md) unencrypted. Since most MUDs do not use encrypted connections, your password
...
Never reuse passwords on multiple services, especially over an insecure connection.
